...
(function(){
  ...
  function applyTheme(){
    ...
    try{ localStorage.setItem('sb-theme', mode); localStorage.setItem('sb-palette', palette); }catch(e){}
    const metaTheme = document.querySelector('meta[name="theme-color"]');
    if(metaTheme) metaTheme.setAttribute('content', mode==='light' ? '#FFFFFF' : '#151821');
  }
  function setThemeMode(mode){ state.themeMode = mode==='light' ? 'light' : 'dark'; applyTheme(); saveState(); }
  function setThemePalette(id){ state.themePalette = PALETTES.some(p=>p.id===id) ? id : 'cyan'; applyTheme(); saveState(); }
  applyTheme();
  ...
  function openAccountModal(){
    ...
    backdrop.innerHTML = `
      <div class="modal">
        ...
        <div class="section-divider"></div>
        <h4 class="modal-subhead">Notifications & reminders</h4>
        <div style="font-size:11px; color:var(--chalk-faint); margin:-4px 0 10px;">Choose which in-app/browser alerts you receive. System notifications also require browser permission.</div>
        <div id="acct-notif-settings"></div>
        <button class="btn btn-ghost" id="acct-enable-push" style="width:100%; margin:8px 0 10px;">🔔 Enable / repair system notifications</button>
        <div class="section-divider"></div>
    ...
    const notifDefs = [
      ['timerEnd','Timer ending','Alert when a focus timer finishes'],
      ['taskApproaching','Task approaching','5-minute reminder before a scheduled task'],
      ['studyStart','Someone starts studying','Alert when someone in your active group starts studying'],
      ['taskComment','Tasks & comments','Alert when someone adds a task or comment'],
      ['groupChanges','Group changes','Alert when someone joins or leaves your group']
    ];
    const notifBox=backdrop.querySelector('#acct-notif-settings');
    notifBox.innerHTML=notifDefs.map(([key,label,desc])=>`<div class="field-row" style="flex-direction:row;align-items:center;justify-content:space-between;gap:10px;"><div style="min-width:0;"><div style="font-size:12px;color:var(--chalk);">${label}</div><div style="font-size:10px;color:var(--chalk-faint);">${desc}</div></div><label class="switch"><input type="checkbox" data-notif-key="${key}" ${notificationSettings[key]?'checked':''}><span class="switch-slider"></span></label></div>`).join('');
    ...
    const pushBtn=backdrop.querySelector('#acct-enable-push');
    pushBtn.addEventListener('click', async()=>{
      pushBtn.textContent='…';
      const r=await enablePushNotifications();
      pushBtn.textContent = r.ok ? '✅ System notifications enabled' : (r.reason==='denied' ? '⚠️ Permission blocked in browser settings' : `🔔 Enable / repair system notifications${r.detail?' — '+r.detail:''}`);
    });
    ...
    backdrop.querySelector('#acct-save').addEventListener('click', async ()=>{
      ...
      const nextNotifSettings = Object.assign({}, notificationSettings);
      backdrop.querySelectorAll('[data-notif-key]').forEach(inp=> nextNotifSettings[inp.dataset.notifKey] = inp.checked);
      await saveNotificationSettings(nextNotifSettings);
      scheduleLocalTaskAlerts();
  ...
  async function loadNotificationSettings(){
    ...
    try{
      ...
      notificationSettings = Object.assign({}, DEFAULT_NOTIFICATION_SETTINGS);
    }
  ...
  async function saveNotificationSettings(next){
    ...
    if(me){
      try{ await setDoc(doc(db,'notificationSettings',me.id), notificationSettings, {merge:true}); }catch(e){ console.warn('notification settings save failed',e); }
    }
  }
  ...
  async function writeTimerSchedule(active){
    ...
    try{
      await setDoc(doc(db,'timerSchedules',me.id), {
        ...
        title: window.__vnextLinkedTaskId ? (window.__vnextLinkedTaskLabel || 'Focus timer') : 'Focus timer',
        updatedAt:Date.now()
      }, {merge:true});
    }catch(e){ console.warn('timer schedule write failed',e); }
  }
